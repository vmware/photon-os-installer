...
class NetworkManager:
    ...
    # should we always use "99-" ?
    SYSTEMD_NETWORKD_PREFIX = "99-"
    ...
    def _convert_legacy_config(self, old_config):
        ...
        if type == 'dhcp':
            ...
            # phys iface same as for tyep 'dhcp', but 'eth0' instead of 'e*'
            config['ethernets'] = {'dhcp-en' :
    ...
    def write_network_file(self, if_id, iface_config, type=IFACE_TYPE_ETHERNET):
        ...
        if 'dhcp4' in iface_config:
            if iface_config['dhcp4']:
                sysdict['Network']['DHCP'] = 'yes'
                sysdict['Network']['IPv6AcceptRA'] = 'no'
